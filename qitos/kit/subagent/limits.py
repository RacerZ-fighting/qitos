"""Root-Run admission limits shared by recursively nested Subagent supervisors."""

from __future__ import annotations

import asyncio
from collections.abc import Iterable
from dataclasses import dataclass

from ...core.subagent import SubagentRunLimitError


@dataclass(slots=True)
class _SubagentRunLease:
    owner: "SubagentRunLimiter"
    committed: bool = False
    released: bool = False

    def commit(self) -> None:
        """Make this launch consume the cumulative budget permanently."""

        if self.released:
            raise RuntimeError("cannot commit a released Subagent Run lease")
        self.committed = True

    async def release(self) -> None:
        """Return the active slot while retaining cumulative launch usage."""

        await self.owner._release(self, rollback=False)

    async def rollback(self) -> None:
        """Return an admission that never became a durable Subagent launch."""

        await self.owner._release(self, rollback=True)


class SubagentRunLimiter:
    """Bound active and cumulative Subagent launches across one recursive Run tree.

    Each nested :class:`SubagentSupervisor` receives the same instance. Admission is
    immediate rather than queued so a foreground Subagent cannot deadlock while it
    waits for a descendant to acquire the last active slot.
    """

    def __init__(self, *, max_active_subagents: int, max_subagents: int) -> None:
        for name, value in (
            ("max_active_subagents", max_active_subagents),
            ("max_subagents", max_subagents),
        ):
            if not isinstance(value, int) or isinstance(value, bool):
                raise TypeError(f"{name} must be an integer")
            if value <= 0:
                raise ValueError(f"{name} must be positive")
        self._max_active_subagents = max_active_subagents
        self._max_subagents = max_subagents
        self._active_subagents = 0
        self._subagents_started = 0
        self._restored_launches: set[str] = set()
        self._lock = asyncio.Lock()

    @property
    def max_active_subagents(self) -> int:
        return self._max_active_subagents

    @property
    def max_subagents(self) -> int:
        return self._max_subagents

    @property
    def active_subagents(self) -> int:
        return self._active_subagents

    @property
    def subagents_started(self) -> int:
        return self._subagents_started

    async def reserve(self) -> _SubagentRunLease:
        """Reserve one active slot and one provisional cumulative launch."""

        async with self._lock:
            if self._subagents_started >= self._max_subagents:
                raise SubagentRunLimitError(
                    "Run Subagent budget exhausted: "
                    f"max_subagents={self._max_subagents}."
                )
            if self._active_subagents >= self._max_active_subagents:
                raise SubagentRunLimitError(
                    "Run active Subagent limit exhausted: "
                    f"max_active_subagents={self._max_active_subagents}."
                )
            self._subagents_started += 1
            self._active_subagents += 1
        return _SubagentRunLease(owner=self)

    async def restore_started(self, launch_ids: Iterable[str]) -> None:
        """Restore durable launches without treating terminal Subagents as active."""

        normalized_items: list[str] = []
        for item in launch_ids:
            if not isinstance(item, str):
                raise TypeError("launch_ids must contain strings")
            normalized = item.strip()
            if not normalized:
                raise ValueError("launch_ids must contain non-empty strings")
            normalized_items.append(normalized)
        normalized_launches = set(normalized_items)
        async with self._lock:
            unseen = normalized_launches.difference(self._restored_launches)
            restored_total = self._subagents_started + len(unseen)
            if restored_total > self._max_subagents:
                raise SubagentRunLimitError(
                    "Restored Subagent launch history exceeds the Run budget: "
                    f"max_subagents={self._max_subagents}, "
                    f"subagents_started={restored_total}."
                )
            self._restored_launches.update(unseen)
            self._subagents_started += len(unseen)

    def reset_for_new_run(self) -> None:
        """Clear cumulative admission state at an owner-Run boundary.

        The composition root calls this only after every supervisor-owned task
        has settled.  Resetting a limiter with an active lease would detach
        that lease from its accounting owner, so it fails closed instead.
        """

        if self._active_subagents:
            raise RuntimeError("cannot reset Subagent Run limits with active subagents")
        self._subagents_started = 0
        self._restored_launches.clear()

    async def _release(
        self,
        lease: _SubagentRunLease,
        *,
        rollback: bool,
    ) -> None:
        if lease.owner is not self:
            raise RuntimeError("Subagent Run lease belongs to another limiter")
        async with self._lock:
            if lease.released:
                return
            if rollback and lease.committed:
                raise RuntimeError("cannot roll back a durable Subagent launch")
            self._active_subagents -= 1
            if rollback:
                self._subagents_started -= 1
            lease.released = True


__all__ = ["SubagentRunLimiter"]
