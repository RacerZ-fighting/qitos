...
You are still inside the same QitOS kernel (the core AgentModule + Engine execution loop), but now the agent must behave well over many steps inside a workspace. This raises new design questions:
...
| State | Plan and cursor | WorkPlan, mode, target file, verification command, optional doc URL |
...
You are learning how much of an agent's behavior can change through:
...
But v0.4 adds one new layer before that transport (the adapter that sends requests to and receives responses from a model API) is created:
...
One example can switch across Qwen, Kimi, MiniMax, `gpt-oss`, and Gemma 4 without changing the agent implementation itself.
...
<Steps>
  <Step title="Start from a preset tool registry">
    ...
    This is the point in the course where presets (reusable configuration bundles) become the right abstraction.
  ...
  <Step title="Use reduce to absorb structured tool output">
    ...
    This is the long-running agent version of the same lesson:
...
You will learn how to specialize:
