...
It is small on purpose, but it is not a toy. You will build a real coding agent with:
...
<Steps>
  ...
  <Step title="Bind the prompt to the parser">
    ...
    In QitOS, this pairing is the harness (the wiring layer that connects a transport, parser, and protocol).
