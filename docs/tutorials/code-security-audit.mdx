...
can the same kernel (the core AgentModule + Engine execution loop) power a serious domain agent without collapsing into custom orchestration?

The answer in QitOS is yes, but only if domain logic goes in the right places.
...
You should only move to a model-specific harness (the wiring layer that connects a transport, parser, and protocol) if the domain actually benefits from it. For example:
...
<Steps>
  <Step title="Compose the tool surface by domain">
    ...
    This is how QitOS specializes agents:
  ...
  <Step title="Use reduce to rank and compress evidence">
    ...
    traces (structured logs of all run events and steps) keep the raw evidence
...
These choices form a coordinated protocol (the output format the model is asked to follow) decision rather than isolated knobs:
...
QitOS keeps parser choice, tool schema choice, and prompt/protocol choice explicit and composable for exactly this reason.
