...
| `timeout_s` | `float` | `None` | Whole-action time budget in seconds |
...
Without `retry_policy`, a tool is invoked exactly once. Validation, approval, and
permission checks are not repeated; all invocation attempts share `timeout_s` and the
Engine runtime deadline.
...
class GetTimeTool(BaseTool):
    ...
    async def execute(self, args: dict, runtime_context: dict | None = None) -> str:
