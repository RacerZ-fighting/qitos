...
`Engine` loop on a daemon worker thread and propagates cancellation cooperatively, so an
unresponsive synchronous call cannot own event-loop or interpreter shutdown.
