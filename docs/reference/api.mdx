...
| `budget_ledger` | `BudgetLedger \| None` | new Root ledger | Shared token/cost owner; pass the Root instance to descendants |
...
| `artifact_store` | `ArtifactStore \| None` | `None` | Persists complete oversized tool output before model projection |
...
Execute the same loop on the caller's event loop. Provider requests and model streams remain async throughout the transaction.
...
Execute the loop and yield `EngineEvent` objects in real time. The stream includes one `run_start` event and ends with one `run_end` event.
...
Request cancellation of the active run. Use `mode="after_step"` to let the current step reach its safe boundary.
When `RuntimeBudget.terminal_synthesis` is enabled, both modes commit a non-empty,
tool-free terminal result. A subsequent resume returns that result without replaying
model or Tool work.
...
`EngineEvent` is the structured event emitted by `Engine.arun_stream()`.
...
class EventStream:
    @property
    def dropped_event_count(self) -> int              # bounded delivery loss
...
| `group` | `str` | Application-defined per-turn exposure group |
| `timeout_s` | `float \| None` | Whole-action timeout in seconds |
| `retry_policy` | `RetryPolicy \| None` | Explicit invocation retry policy |
...
```
Capture a read-only registry snapshot for one model turn. Engine uses the same
exposure for tool projection and action dispatch.
...
Lookup is exact. Execution belongs to `ActionExecutor`, which applies validation,
permissions, timeouts, retries, and canonical result handling before calling
`BaseTool.execute(args, runtime_context)`.
...
| `cancelled_after_step` | The Engine committed the current step before stopping; the trace manifest uses terminal status `stopped` |
