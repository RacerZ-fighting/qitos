...
- `SkillToolSet(...)` for read-only bundled Skill disclosure and optional
  provider-backed Skill management
...
- warning thresholds before the Engine's 80% force boundary
- transaction-aware round grouping with `MessageGrouper`
- level 1 micro-compaction of large messages in older complete rounds
...
schemas, against a provider-safe input budget. At 80% it requests compaction before model
dispatch. Summary work runs against an immutable history snapshot and is committed only
...
resources. The same toolset retains QitOS's separate provider-backed search, install,
and activation tools.

```python
from qitos import ToolRegistry
from qitos.kit.tool import SkillToolSet
...
Launch a fresh child-agent run from a normal tool call. The owning application must
inject an invocation factory that constructs the child from its run-scoped resources.

```python
from qitos.core import ChildInvocation, TaskBudget
from qitos.kit.tool.agent import AgentTool, ChildControlToolSet

async def build_child(request, runtime_context):
    model = await make_fresh_model()
    child_engine = make_fresh_engine(
        model=model,
        delegate_depth=runtime_context.delegate_depth + 1,
    )
    return ChildInvocation(engine=child_engine, task=request.task)

tool = AgentTool(
    invocation_factory=build_child,
...
resolve to an independent Engine; sharing a mutable AgentModule, history, or model
client between concurrent children is unsupported.
...
The async factory returns a `ChildInvocation`; live Engine objects never enter persisted child
...
`agent.child.completed` `RuntimeInput` to the exact parent Engine run when the child
...
its launch context without allocating a model, Engine, or trace before it can run. If a
...
The Engine supplies the async callback through
...
It also appends `child.started` before constructing the child Engine and
`child.terminal` before delivery. Resume converts a started child without a terminal
record to `interrupted` without replaying the Engine. Fork recovery ignores inherited
...
`aclose()` cooperatively cancels outstanding child Engines; applications with run-scoped
...
child that has not entered its Engine yet.
...
Message delivery uses the Child Engine's durable mailbox and becomes visible only at a
...
Resolve environment configuration separately, then construct from an explicit factory.
...
`ModelConfig.from_env()` reads provider environment variables; `build_model()` uses an
isolated built-in factory unless one is supplied explicitly.
