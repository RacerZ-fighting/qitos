...
- `UpdateWorkPlanTool` / `update_plan` for one typed, checkpoint-friendly checklist
...
`final_answer` normally contains text. A JSON object or array is also accepted and
is normalized to compact JSON text in `Decision.final_answer`, preserving the core
string contract for state and trace consumers.
...
- an 80% observability warning and the Engine's explicit 85% compaction boundary
- transaction-aware round grouping with `MessageGrouper`
- boundary-triggered micro-compaction of large messages in older complete rounds
- level 2 summaries that preserve configured recent rounds
- level 3 resummaries that preserve the latest complete round
...
`MessageGrouper` groups messages into compactable rounds. It correlates assistant calls
and tool results by call id even when they cross step boundaries, so selection and
compaction never split one tool transaction.

The Engine counts the complete provider packet, including API-delivered tool and response
schemas, against a provider-safe input budget. At 80% it emits a warning without changing
messages; at 85% it requests compaction before model dispatch. Summary work runs against
an immutable history snapshot and is committed only
as a projection when the version still matches; canonical history remains untouched.
Exact-prefix checkpoints let later projections summarize only newly covered complete
rounds. With auto-compaction enabled, token selection precedes generic message-count
windows. Use `keep_last_rounds` for transaction-safe recent retention; compact history
does not expose arbitrary message slicing that could split provider tool transactions.
...
allowed tool groups, and working-directory fields needed by a product composition.
...
that argument, always returns a running receipt with a parent-scoped `ChildHandle`, and posts an
`agent.child.completed` `RuntimeInput` to the exact parent run when the child
terminates. `AgentTool` snapshots the parent history at launch, then delegates lifecycle
ownership to a Run-scoped `ChildSupervisor`. The supervisor does not call the invocation
factory until `execution_scope` admits the task. Queued work therefore retains
...
`ChildLaunchContext.post_runtime_event`. Child supervision stores its terminal result
before awaiting that callback; on Journal-backed parents, successful delivery is
durable before the parent wakes.
...
If execution stops between `child.terminal` and mailbox acceptance, resume derives the
same stable completion input from the terminal fact and delivers it once for a background
invocation. Foreground results are returned only through the original Agent ToolResult.
...
parallel only when the parent Engine's `ActionExecutionPolicy` allows `Agent` and the
