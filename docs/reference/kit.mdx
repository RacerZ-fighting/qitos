...
`subagent_type`. Each factory call must return an independent Engine; sharing a mutable
AgentModule, history, or model client between concurrent children is unsupported.
...
The factory may await run-scoped resource construction and returns a `ChildInvocation`;
existing synchronous factories remain accepted. Live Engine objects never enter
persisted child requests or results. If the invocation creates a model client, trace, or
other live resource, set its async `cleanup` callback. The supervisor awaits cleanup
after success, failure, or cancellation before publishing the terminal result. If
construction fails before returning the invocation, the factory still owns and must
asynchronously close anything it already created before propagating that error.
...
The Engine supplies an async `runtime_context["post_runtime_event"]` callback. Child
supervision stores its terminal result before awaiting that callback; on Journal-backed
parents, successful delivery is durable before the parent wakes.
...
`runtime_context["agent_cancelled"]` callback so teardown can wake a child that has not
entered its Engine yet.
