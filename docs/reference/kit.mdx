...
terminates. `AgentTool` snapshots the parent history at launch, but does not call the
invocation factory until `execution_scope` admits the task. Queued work therefore retains
...
`active_background_count`, `child_result(handle)`, `cancel_child(handle)`,
...
again after an earlier non-blocking cancellation request. Background children are
event-loop-owned asyncio tasks; teardown cancels and drains every task. Terminal result
state is stored before QitOS attempts to
wake the parent. An `execution_scope` that waits for an application slot should observe the
