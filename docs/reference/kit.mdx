...
apply it through the executor before invoking the tool.
...
`snapshot_background_events()`, and `close()` expose lifecycle control to the owning
...
`close()` cooperatively cancels outstanding child Engines; applications with run-scoped
...
again after an earlier non-blocking cancellation request. Background children use a
bounded daemon worker pool, so an uncooperative synchronous child cannot keep interpreter
shutdown alive indefinitely. Terminal result state is stored before QitOS attempts to
