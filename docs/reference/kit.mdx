...
windows. `keep_last_messages` is deprecated and accepted only as a no-op compatibility
argument; use `keep_last_rounds`.
...
Launch a fresh child-agent run from a normal tool call. Applications that own
run-scoped resources should inject an invocation factory rather than relying on the
legacy class registry.
...
`AgentRequest.max_turns`; legacy class-registry children receive it through
`AgentModule.run(max_steps=...)`.
...
When `allow_background=False` (the compatibility spelling for foreground mode), the
background argument is omitted from the schema.
