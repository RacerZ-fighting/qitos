...
def build_child(request, runtime_context):
    ...
    return AgentInvocation(engine=child_engine, task=request.prompt)
...
`max_turns` is a tool-level safety ceiling and defaults to the exported
`DEFAULT_SUBAGENT_MAX_TURNS` value of 200. Invocation factories receive that value on
`AgentRequest.max_turns`.
...
that argument, always returns a running receipt with a task id, and posts an
...
`active_background_count`, `get_background_result(task_id)`, `cancel_background(task_id)`,
`snapshot_background_events()`, and `aclose()` expose lifecycle control to the owning
application. A snapshot projects only the active children's bounded, completed tool
