...
`auto_approve=True` allows commands that the shell analyzer classifies for review and
removes the tool-level approval marker. Commands classified as destructive remain
blocked unless the call explicitly sets `allow_destructive=True`. The standalone
`qitos.kit.tool.shell.RunCommand` accepts the same option.
