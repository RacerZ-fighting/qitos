...
def build_child(request, runtime_context):
...
The factory returns a `ChildInvocation`; live Engine objects never enter persisted child
...
failure, or cancellation before publishing the terminal result.
