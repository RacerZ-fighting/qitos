...
terminates. Invocation factories run before the background submission so parent context
is frozen at launch. If a child has no final answer, the completion payload contains a
bounded projection of its recent tool evidence instead of silently dropping useful work.
...
resources should call it before those resources leave scope. An `execution_scope` that
waits for an application slot should also observe the
