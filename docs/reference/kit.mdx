...
Expose a bounded catalog from application-owned, read-only Skill roots and load one
selected `SKILL.md` without truncating its content. The same toolset retains QitOS's
separate provider-backed search, install, and activation tools.
...
# Applications that only allow bundled assets register exactly these two tools.
registry = ToolRegistry()
...
snapshot. These paths do not invoke a provider, install a package, or create a
workspace registry.
