...
Refresh validates every immediate child containing `SKILL.md`, rejects duplicate
manifest names, and replaces the previous in-memory snapshot only after the entire
catalog succeeds. `list_skills` returns stable name-sorted summaries with a bounded
limit; `load_skill` requires one exact catalog name and returns the complete validated
snapshot, its SHA-256 content revision, declared requirements, and stable relative
resource identifiers. `read_skill_resource` accepts only one of those identifiers,
rechecks the resolved path against the selected Skill root, and returns an opaque
content-bound cursor when a UTF-8 resource needs another page. Applications can use
`bundled_skill_snapshots()` to persist and later revalidate loaded instructions. These
paths do not invoke a provider, install a package, or create a workspace registry.
