...
attempt starts after either becomes terminal. A synchronous provider that cannot be
interrupted is detached on a daemon worker; its late response is discarded and cannot
update history, actions, or final state.
...
stalled response. Direct callers of the public
