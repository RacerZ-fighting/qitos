...
|---|---:|---|
| `timeout` | `120` | Total request timeout for one transport attempt |
| `max_attempts` | `2` | Total attempts including the first request |
| `stream_idle_timeout` | `60` | Maximum seconds between async Responses stream events |

Synchronous calls and async Responses streams activate this policy. QitOS creates the
corresponding OpenAI-compatible SDK client with `max_retries=0` and becomes the only
...
and ordinary 4xx responses fail immediately. Async Responses streams are retried only
before the first event and are closed before a retry, avoiding duplicated partial
output.
