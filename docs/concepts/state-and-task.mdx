...
### Lightweight WorkPlan

Use `WorkPlanState` when a long-running agent needs a small ordered checklist. It is a
typed state value, not a task board and not free-form metadata. The `update_plan` tool
returns a complete validated replacement; your AgentModule applies successful results
...
from qitos import Action, StateSchema, ToolResult, WorkPlanState, reduce_work_plan
...
Use `work_plan_state_to_dict()` and `work_plan_state_from_dict()` in your StateSchema
codec so checkpoints reconstruct the typed value. `render_work_plan_markdown()` creates
a disposable TODO projection; never read that projection back as canonical state.
...
| ------------------ | ----------------------- | ----------------------------------------------------------------- |
| `id`               | `str`                   | Stable task or benchmark-sample identifier.                       |
...
`Task.id` identifies the task, not one execution of it. A plain-text task is wrapped
with a content-derived stable ID. Trace and Journal `run_id` values identify separate
execution instances, including repeated trials of the same task.
