...
After a model call, the Engine records the normalized response and gives `AgentModule.interpret_model_response()` the first opportunity to turn it into a decision. If the hook returns `None` and the response contains neither non-whitespace text nor tool calls, the Engine classifies the result as a `model_error` before it reaches the parser. The default recovery path retries once; a second consecutive empty response stops the run with `unrecoverable_error` instead of consuming the remaining step budget as parser `wait` decisions.
