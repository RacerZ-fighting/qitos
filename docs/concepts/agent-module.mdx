...
For a tool-using agent, set `runtime_context` with
`runtime_context_delivery="merge_tool"` to attach transient controller state
to the final real tool result. QitOS keeps its original `tool_call_id` and adds
a notice that this state is not tool output. If no text tool result is available
or a multimodal user turn is required, QitOS safely falls back to a user
message; it never writes the transient context back to history.

```python
return MessageBuildResult(
    messages=messages,
    runtime_context=current_state_summary,
    runtime_context_delivery="merge_tool",
)
```
