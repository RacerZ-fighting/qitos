...
`AgentModule` is the contract you implement to define agent behavior. It owns everything about how your agent thinks — state initialization, prompt construction, decision logic, and state reduction — but it never runs the loop itself. That is the [Engine's](/concepts/engine) job.
...
Two hooks are **required** (`init_state` and `reduce`). The remaining hooks are
optional and keep the minimal path small.
...
Engine calls `prepare_turn(state, observation, turn)` after it freezes the current
`TurnSnapshot`. The default delegates to `prepare(state)`. Override `prepare_turn` only
when prompt projection needs the explicit protocol, History, Tool, capability, or
remaining-budget view; do not read temporary attributes from the Agent instance.

### Custom message builders

Agents that need an exact provider message shape can expose a `message_builder`
implementing `qitos.core.message_builder.MessageBuilder`. Its
`MessageBuildResult` owns outbound messages and optional history entries.

Use `ContextSnapshot` for an application-state projection that must survive
resume and remain cache-stable. QitOS appends a changed revision as its own
canonical user message, persists exactly what the model saw, and leaves every
earlier user, assistant, and ToolResult message unchanged. Returning the same
revision and digest is a no-op; reusing one revision for different content is a
configuration error. The snapshot is a model-input fact, not a replacement for
the application's typed state.

```python
from qitos import ContextSnapshot

return MessageBuildResult(
    messages=messages,
    context_snapshot=ContextSnapshot(
        revision=str(state.revision),
        content=current_state_summary,
    ),
)
```
...
### `assess_completion` (optional)

A model-proposed final answer is a proposal, not an unconditional state mutation.
Override `assess_completion(state, decision)` when product state decides whether the
objective is actually complete:

```python
from qitos.core import CompletionAssessment

def assess_completion(self, state, decision):
    if state.blocker:
        return CompletionAssessment.blocked("external prerequisite")
    if not state.verified:
        return CompletionAssessment.continue_run(
            "Verify the result before concluding.",
            reason="verification_missing",
        )
    return CompletionAssessment.completed("verified")
```

The default accepts the proposal. Accepted and blocked answers use distinct stop
reasons. A rejected proposal appends the feedback to canonical History and starts a new
turn without retaining `final_result`.

### `build_tool_exposure` (optional)

Return a `ToolExposure` that freezes the exact definitions available to one turn. The
same exposure drives model schema projection and Tool dispatch. Registry or policy
changes made during the request can affect only the following turn.
...
| `critics`       | `list[Critic]`        | Critics (post-step evaluators that can stop or retry) evaluated after each step |
