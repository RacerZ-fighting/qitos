...
<Warning>
QitOS is currently **Beta**. Current releases ship the minimal agent loop and the `Agent` façade. Trace/qita behavior and canonical examples remain supported, but pin your version in production and review the changelog before upgrading.
</Warning>
...
The current core install includes the minimal agent loop and `Agent` façade, the `qita` observability tools (QitOS's built-in trace viewer), and the `kit` toolkit modules (pre-built toolsets, memory adapters, and environments). It does not include the OpenAI client.
