...
<Warning>
QitOS is currently **Beta**. Current releases ship `AgentModule + Engine`; the accepted runtime migration will replace that API in place. Trace/qita behavior and canonical examples remain supported, but pin your version in production and review the changelog before upgrading.
</Warning>
...
The current core install includes the shipped `AgentModule + Engine` runtime, the `qita` observability tools (QitOS's built-in trace viewer), and the `kit` toolkit modules (pre-built parsers, memory adapters, toolsets, critics, and planners). It does not include the OpenAI client or benchmark dataset loaders.
...
| Extra | What it adds |
|---|---|
| `models` | `openai>=1.66.0` — required for Chat Completions and optional Responses API model endpoints |
...
You can also verify both CLIs are on your path:
