...
模型调用完成后，Engine 会先记录规范化响应，并优先让 `AgentModule.interpret_model_response()` 将其解释为决策。如果该钩子返回 `None`，且响应既没有非空白文本也没有工具调用，Engine 会在进入 parser 之前将其归类为 `model_error`。默认恢复路径会重试一次；若连续第二次仍为空，运行会以 `unrecoverable_error` 停止，而不是继续生成 parser `wait` 并消耗剩余步数预算。
