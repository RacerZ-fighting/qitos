---
title: "Agent Module"
description: "AgentModule 是策略层，用来定义智能体如何初始化状态、构造提示词、做决策，并把观测结果归约到下一步状态。"
---

`AgentModule` 是你定义智能体行为时要实现的核心契约。它负责智能体如何思考的全部内容：状态初始化、提示词构造、决策逻辑以及状态归约，但它**不执行循环本身**。执行循环由 [Engine](/zh/concepts/engine)（执行内核）负责。
...
| `ObservationT` | Engine 传给各个钩子的观测结果（每步后智能体接收的结构化观察结果）类型 |
| `ActionT` | `decide` 产生的动作（标准化工具调用）类型 |
...
`AgentModule` 定义了六个钩子。其中两个是**必需的**：`init_state` 与 `reduce`。其余钩子都是**可选的**，并且有合理默认值。

### `init_state`（必需）

每次运行开始时调用一次。你需要返回一个初始状态对象，通常基于任务字符串初始化。
...
每次动作循环结束后调用。它把旧状态、新观测结果和本步决策归约成下一步状态——这是智能体从执行结果中学习的地方，也是归约的核心机制。
...
<Warning>
  `reduce` 必须返回 `state` 或一个新的状态对象。返回 `None` 会触发运行时错误。
</Warning>

### `build_system_prompt`（可选）

返回动态系统提示词；如果返回 `None`，则本步不使用系统提示词。它会在每个决策步骤之前调用，因此提示词可以根据当前状态动态变化。
...
把当前状态转成模型可直接消费的文本，也就是用户轮次。默认行为是 `str(state)`。通常你会覆写它，把多个状态字段组织成结构化输入。
...
如果你想在 Engine 走默认大模型调用路径之前插入自定义决策逻辑，就覆写它。返回 `None` 时，Engine 会继续走模型调用与解析器路径。

```python
def decide(self, state: StateT, observation: ObservationT) -> Decision[ActionT] | None:
    if state.current_step == 0:
        return Decision.wait("先检查当前编码上下文")
    return None
```

### `should_stop`（可选）

在每步 `reduce` 之后额外检查一次停止条件。返回 `True` 会终止运行。Engine 的内置停止条件仍会继续叠加生效。
...
### `assess_completion`（可选）

模型给出的 final 是完成提议，不应直接等价于产品完成。产品可以覆写
`assess_completion(state, decision)`，返回 `CompletionAssessment.completed()`、
`blocked()`，或带反馈的 `continue_run()`。继续运行时，反馈写入 canonical History，
`final_result` 被清空；完成和阻塞使用不同的 stop reason。

### `prepare_turn` 与 `build_tool_exposure`（可选）

每轮开始时 Engine 会冻结一个 `TurnSnapshot`。需要读取显式协议、History、Tool、能力或
剩余预算时覆写 `prepare_turn`，不要在 Agent 上保存临时 `_runtime_*` 属性。
`build_tool_exposure` 返回的同一个冻结视图同时用于模型 schema 和执行分发；本轮中发生的
注册表或策略变化只能影响下一轮。

### 自定义消息构造器与状态快照

需要精确控制 Provider 消息时，Agent 可以提供实现
`qitos.core.message_builder.MessageBuilder` 的 `message_builder`。如果当前应用状态也需要进入
模型上下文，使用 `ContextSnapshot` 返回一个带稳定 revision 的只读投影：
...
QitOS 只在 revision 或内容 digest 变化时追加一条独立的 canonical user message，并持久化模型
实际看到的文本；既有 user、assistant 与 ToolResult 都不会被回写。同一 revision 与 digest 不会
重复追加，同一 revision 对应不同内容会直接报配置错误。Snapshot 是模型输入事实，不代替应用的
类型化状态真源。

## 一个最小可运行示例

在 QitOS 里，公开的最小示例依然是一个真实的编码智能体：它会配置模型、挂载工作区，并让 Engine 驱动工具使用与验证。
...
调用 `agent.run()` 即可执行一个任务。这是最常用的入口。它会在内部创建 `Engine`、运行循环，并返回结果。
...
| `workspace` | `str \| None` | 挂载到宿主环境的工作目录 |
| `return_state` | `bool` | 返回 `EngineResult`，而不是 `state.final_result` |
| `trace` | `bool \| TraceWriter` | `True` 自动启用追踪记录，`False` 关闭，或直接传入 `TraceWriter` |
| `trace_logdir` | `str` | 追踪记录输出目录，默认 `"./runs"` |
| `parser` | `Parser` | 与提示词格式匹配的输出解析器（将原始模型输出解析为 Decision 的组件） |
| `critics` | `list[Critic]` | 每步执行后评估的评估器（可批准、停止或重试当前步骤） |
| `stop_criteria` | `list[StopCriteria]` | 除默认条件外的附加停止条件 |

<Tip>
  如果你需要更细粒度的控制，例如自定义钩子、分支选择器，或在多次运行之间复用同一个 Engine，可以使用 `agent.build_engine(**kwargs)`，然后直接调用 `engine.run(task)`。
</Tip>
...
```python
AgentModule(
    tool_registry=registry,   # ToolRegistry：智能体可用的工具
    llm=model,                # 大模型客户端，供 Engine 默认模型路径使用
    model_parser=parser,      # 把原始模型输出解析成 Decision 的解析器
    memory=memory,            # 跨步或跨运行的记忆实现
    history=history,          # 单次运行内的历史实现
