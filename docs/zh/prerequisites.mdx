---
title: "前置准备"
description: "注册远程模型服务、获取 API 密钥，并在运行 QitOS 前完成基础连通性验证。"
---

在运行任何依赖大语言模型（LLM）的 QitOS 示例之前，你需要先准备一个远程模型 API。

QitOS 对兼容 OpenAI 的服务商（provider）适配最为直接。只要你能拿到以下信息，通常就能开始使用：
...
这两个平台都提供兼容 OpenAI 的接口。注册流程、价格与试用额度会随时间调整，请以控制台和官方计费页的最新信息为准。
...
准备好远程模型后，继续阅读 [快速开始](/zh/quickstart)。
