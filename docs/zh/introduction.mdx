...
当前 QitOS 版本通过同一套 `AgentModule + Engine` 运行时构建可复现的 LLM 智能体（agent）。无论你是在原型化 ReAct 循环、运行 GAIA 基准测试（benchmark），还是交付一个面向真实工作流的编程智能体，运行结果始终可追踪、可组合、可复现。已经确认的运行时重构会原地将这套 API 替换为 Model、最小异步 Agent loop 和可恢复 Session/Harness。
...
<Warning>
QitOS 当前处于 **Beta** 阶段。运行时迁移期间继续保持 trace/qita、canonical durable records、规范化示例和 benchmark adapter 的行为；`AgentModule + Engine` 被原地替换时，公开 Agent/runtime API 会变化。用于生产环境时请固定版本并关注 Changelog，不要把当前 API 视为冻结接口。
</Warning>
