---
title: "第 4 课：代码安全审计智能体"
description: "把 QitOS 内核专门化成一个可复现的防御式审计智能体，具备领域工具、排序发现与审查级追踪记录。"
---

最后一课会回答整条课程里最难的问题：

**同一个执行内核，能不能支撑一个真正严肃的领域智能体，而不坍缩成定制编排？**
...
- 你需要更严格机器可读输出时，改用 JSON 或 XML
- 供应商原生结构化工具调用更可靠时，使用原生工具调用解析器
- 智能体驱动的是交互式终端而不是仓库工具时，使用 Terminus 风格协议

QitOS 支持这些路径，但默认研究路径依然是与供应商无关的文本 ReAct。

<Steps>
  <Step title="按领域组合工具界面">
    这一课会组合三类工具：
    ...
    这就是 QitOS 希望你用来专门化智能体的方式：

    **组合环境，而不是重写循环。**
  </Step>

  <Step title="让提示词与 prepare 共同编码审计方法">
    提示词负责定义全局审计纪律，而 `prepare()` 负责定义当前阶段的局部框架：
  ...
  <Step title="让归约负责证据压缩与排名">
    示例里通常只会从工具输出中抽取最高信号的发现：
  ...
  <Step title="使用有限历史，而不是无限累积">
    ...
    这是一个很强的默认值，因为它既能保留最近的推理与证据，又不会让模型每步都重新阅读大量历史搜索结果。

    如果你准备把审计扩展成更大的工作流，下一个升级通常是 `CompactHistory`，而不是无上限历史。
  </Step>

  <Step title="只有当记忆真能改变结果时再加它">
    这一课默认仍然不挂载单独记忆适配器。

    对教程来说，这是正确的：

    - `findings` 已经承担了压缩型状态记忆的角色
    - `qita` 会保留完整追踪记录供后续审阅
    - 额外检索层只会让课程复杂化
  ...
  <Step title="把 qita 当成审查产物，而不只是调试器">
    运行：
    ...
    这一课里，`qita` 的角色已经超过调试器：

    - 它帮助你检查智能体是否先做盘点再下结论
    - 它帮助你看哪些发现被提升到了 `state.findings`
    - 它帮助你确认解析器诊断是否稳定
    - 它帮助你判断上下文压力是否影响了审计质量
    - 它帮助你把最终答案读成一份排序审查报告，而不是一堆原始搜索命中
  </Step>
</Steps>
...
Whitzard 的价值不只是它是一个更强的审计智能体，更在于它让一个更普遍的设计原则变得具体：

**当模型有很强的原生工具调用先验时，适配脚手架往往比强迫所有模型走同一协议更合理。**
...
<CardGroup cols={2}>
  <Card title="构建你自己的智能体" icon="sparkles" href="/zh/guides/build-your-first-agent">
