...
| `timeout_s` | `float` | `None` | 整个 action 的时间预算（秒） |
| `retry_policy` | `RetryPolicy` | `None` | 显式调用重试策略 |
| `read_only` | `bool` | `False` | 标记工具为无副作用 |
| `concurrency_safe` | `bool` | `False` | 表示工具可安全并行执行 |

未配置 `retry_policy` 时，工具只调用一次。输入校验、审批与权限检查不会重复，
所有调用 attempt 共享 `timeout_s` 和 Engine 的运行 deadline。
