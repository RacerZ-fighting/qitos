...
两者都不是必选项。如果你两者都不传，Engine 会使用内部历史、默认的 24 条消息 `HistoryPolicy` 上限，并且不启用记忆适配器。默认开启上下文管理时，内部存储会升级为 `CompactHistory`；关闭上下文管理时，则保留轻量的 Engine 窗口存储。
...
`HistoryPolicy.build_query()` 会生成发给当前 `History` 适配器的检索查询。默认开启上下文管理时由 `CompactHistory` 处理；关闭上下文管理时，轻量的 `_EngineWindowHistory` 后备实现会满足同一查询。
...
消息上限仍然按单条历史消息计数，因此窗口边界可能落在一次原生工具调用轮次内部。发送模型请求前，Engine 会通过 `tool_call_id` 关联 assistant 工具调用和工具结果：如果声明调用的 assistant 消息已经不在窗口内，Engine 会移除对应的孤立工具结果；如果保留的 assistant 调用缺少结果，则继续使用原有的中断占位结果。合法完整调用链的顺序和内容保持不变，配置的消息上限也不会改变。
...
`1.0` 表示在 Engine 的 85% 明确水位之前不改写投影历史，从而在两次压缩之间保持
append-only Provider 前缀，同时让每次压缩都能在 trace 中被观察。
