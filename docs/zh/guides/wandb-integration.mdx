...
`add_trace_processor` 会将处理器追加到全局列表，因此可以将 `WandbTraceProcessor` 与其他 `TraceProcessor`（例如用于 JSON 导出的 `JsonFileTraceProcessor`）组合使用：
...
# 当前 provider 同时启用 W&B 和 JSON 处理器
result = agent.run(task="...", return_state=True)
```
