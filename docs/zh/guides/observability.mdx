...
| 文件 | 内容 |
|---|---|
| `manifest.json` | 运行元数据：`run_id`、`model_id`、`status`、`stop_reason`、`step_count`、`event_count`、`schema_version`、`prompt_hash`、`task_hash`、`run_config_hash`、`seed`、`summary` |
...
`manifest.json` 会在初始化时写出，并在运行结束时完成定稿。`events.jsonl` 与 `steps.jsonl` 都是只追加写入。

`task_hash` 对完整的 canonical Task 包计算指纹，其中包含稳定的任务或样本 ID。
纯文本任务使用内容生成 ID，不再使用挂钟时间；trace `run_id` 才是执行实例标识。
`run_config_hash` 单独标识运行配置。qita 只有在两者都完整且匹配时才会声明
same-spec；缺少 `task_hash` 的旧 trace 仍可读取，但其比较 provenance 会被标为不完整。
...
qita 会优先按 `action_id`，否则按记录的原始顺序把每个动作与结果配对。工具统计复用
同一配对，因此一个失败结果只会归到对应调用；每个精确生命周期状态都会单独计数，
`unmatched_actions` 与 `unmatched_results` 则直接暴露不完整的 trace 闭包，不会猜测归属。
...
### 使用 Agent façade 订阅事件

对于实时运行，可以通过 façade 的生命周期事件订阅以编程方式消费事件：
...
result = await agent.prompt("分析数据")
```
