...
```bash
pytest --cov=qitos.core --cov=qitos.trace --cov-report=term -q
```
...
Security-sensitive tools must require explicit opt-in imports and must not be exposed from `qitos`, default `qitos.kit` imports, or quickstart examples.

### Adding A Product Agent

Keep QitOS examples small, teaching-first, and focused on one framework concept at a time; product-grade agents belong in downstream product repositories.
