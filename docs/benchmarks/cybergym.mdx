...
CyberGym uses the generic Engine envelope: 8,000 model-visible characters per result and
16,000 characters for one Tool batch. Complete long `READ` and `BASH` output is persisted
first; the model receives a deterministic head/tail receipt, path/size facts, and an
Artifact reference it can read in bounded chunks. Benchmark code does not raise the
prompt limit to carry raw exploit-development transcripts inline.
