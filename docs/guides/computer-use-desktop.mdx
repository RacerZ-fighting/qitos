...
QitOS v0.5 now has a first official provider-neutral **computer-use lane** (a complete execution path for GUI and desktop interaction) for desktop and GUI work.

This lane is inspired by the original OSWorld architecture and task loop, but it is implemented in QitOS-native pieces:
...
## Environment smoke example

The smallest environment-only loop is:

```bash
python examples/real/desktop_env_smoke.py
```

It exercises `DesktopEnv` and the computer-use toolset without a model.

## Container-first direction

The desktop lane is designed to be **container-first**.
This keeps desktop runs aligned with:

- reproducible desktop state
- provider isolation
- clean env lifecycles

The qita visual inspection surface covers screenshot-backed runs.
