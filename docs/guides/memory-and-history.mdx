...
`auto_compact=False` restores count-window selection. The old
`keep_last_messages` argument remains temporarily accepted for compatibility but no
longer slices messages; use `keep_last_rounds` for transaction-safe retention.
