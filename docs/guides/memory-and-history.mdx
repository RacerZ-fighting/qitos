...
Message limits still count individual history messages, so a window boundary can fall inside a native tool-call round. Before dispatching a model request, the Engine correlates assistant tool calls and tool results by `tool_call_id`. It removes a tool result when the declaring assistant call is no longer present and keeps the existing interrupted-call placeholder for an assistant call whose result is missing. Valid complete chains keep their original order and content, and the configured message limit does not change.
