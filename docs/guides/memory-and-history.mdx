...
- an observability warning at 80% and an explicit Engine compaction boundary at 85%
- transaction-aware grouping of generic and provider-native tool rounds
- boundary-triggered micro-compaction for large messages in older complete rounds
...
- micro-compaction — trims very large old messages only after the explicit boundary
...
At 80%, Engine emits a warning without changing messages. At 85%, model dispatch is
blocked until the history projection is compacted. If the
...
The `1.0` ratios keep the projected transcript unchanged until the Engine's explicit
85% watermark. This preserves an append-only Provider prefix between compactions while
keeping every compaction event visible in traces.
