...
```python
    def reduce(
        ...
        decision: Decision[Action],  # the structured output from the model
    ) -> MinimalCodingState:
...
| Wrong prompt-parser pairing | Parse errors every step | Match your system prompt format to `ReActTextParser` (a parser that extracts structured Decisions from ReAct-formatted text) or another explicit parser |
