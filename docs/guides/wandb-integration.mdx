...
`add_trace_processor` appends to the global processor list, so you can combine `WandbTraceProcessor` with any other `TraceProcessor` (for example, `JsonFileTraceProcessor` for a JSON export):
...
add_trace_processor(JsonFileTraceProcessor(output_dir=".traces", streaming=True))

# The provider now has both the W&B and JSON processors.
result = agent.run(task="...", return_state=True)
```
