...
`manifest.json` is written on initialization and finalized when the Engine exits.
...
### Streaming with Engine

For live runs, use `Engine.arun_stream()` to consume events programmatically:

```python
from qitos import Engine

engine = Engine(agent=agent, budget=RuntimeBudget(max_steps=10))
