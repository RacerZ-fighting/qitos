---
title: "Method Recipes"
description: "Use maintained method recipes (Self-Refine, Reflexion, LATS, MoA, Magentic-One) to implement proven agent patterns with minimal boilerplate."
---

QitOS provides **method recipes** — importable Agent + Critic pairs that implement
well-known agentic reasoning patterns. Each recipe packages a specialized state,
critic, and agent so you can apply the pattern without rewriting the control loop.

## What is a method recipe?

A method recipe combines:
...
This recipe owns the MoA method state, not a second Child runtime. Applications that
need parallel proposers should expose QitOS `AgentTool` instances through an
application-owned invocation factory and retain their own role policy.
...
## Building your own method recipe

To create a custom method recipe, follow the same Agent + Critic pattern:
