...
The point is no longer just laying a multimodal foundation.
...
A complete visual inspection path stays small:

1. create a screenshot-backed task
2. expose a screenshot-first env (`ScreenshotEnv`)
...
`examples/real/desktop_env_smoke.py` exercises the environment half of this path
without a model and is a better starting point for visual-web / GUI research than
jumping straight into a benchmark harness.
...
- screenshot observations now fit the loop/trace (structured run log)/qita model
- GUI capability hooks now exist in the environment contract

So desktop adapters can be built **on top of** the minimal agent loop instead of beside it.
