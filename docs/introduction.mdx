...
Current QitOS releases use one `AgentModule + Engine` runtime for building reproducible LLM agents. Whether you are prototyping ReAct loops, running GAIA benchmarks, or shipping a production coding agent, QitOS keeps your runs traceable, your patterns composable, and your results reproducible. The accepted runtime refactor will replace that API in place with a Model, minimal async Agent loop, and durable Session/Harness.
...
Product-style agents, benchmark runners, and domain agents all reuse the same runtime. Full product applications belong in `qitos-zoo`; domain behavior is expressed through:
...
<Warning>
QitOS is currently **Beta**. Trace/qita behavior, canonical durable records, examples, and benchmark adapters remain supported during the runtime migration. The public Agent/runtime API will change as `AgentModule + Engine` is replaced in place, so pin the installed version and follow the changelog rather than treating that API as frozen.
</Warning>
