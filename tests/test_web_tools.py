...
@pytest.mark.asyncio
async def test_http_request_parses_json(monkeypatch):
    tool = HTTPRequest()
    ...
    monkeypatch.setattr(tool._client, "request", _fake_request)
    out = await tool.execute({"method": "GET", "url": "https://example.com"})
...
@pytest.mark.asyncio
async def test_http_get_and_post_delegate(monkeypatch):
    get_tool = HTTPGet()
    post_tool = HTTPPost()

    calls = []

    async def _fake_execute(args, runtime_context=None):
        _ = runtime_context
        calls.append(args)
        return {
            "status": "success",
    ...
    g = await get_tool.execute({"url": "https://a.com", "params": {"q": "x"}})
    p = await post_tool.execute({"url": "https://b.com", "json_data": {"x": 1}})
    assert g["method"] == "GET"
...
@pytest.mark.asyncio
async def test_html_extract_text_emits_no_deprecation_warning():
    tool = HTMLExtractText()
    html = "<html><head><title>Demo</title></head><body><p>Hello</p></body></html>"

    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        out = await tool.execute({"html": html, "max_chars": 1000})
