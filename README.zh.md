...
- **只保留一条 canonical 可观测路径**：重复的进程级 tracing provider 及其 W&B/MLflow
  ...
  `HANDOFF_START`/`HANDOFF_END` 事件完整可见。已废弃的 debug 包、trace 文件“fork”和
  checkpoint snapshot 复制式 fork 也已删除；可继续执行的 fork 统一使用已提交的 Journal
  lineage。
...
quickstart 通过 `agent.prompt(...)` 驱动 `Message -> Model -> ToolCall -> ToolResult` 循环，并打印最终助手消息。

接下来可以继续：

- 想看构建块：见 [kit 参考](https://qitor.mintlify.app/zh/reference/kit)
...
安全敏感工具为显式 opt-in 导入，不在 `qitos`、`qitos.kit` 或快速开始路径中。
...
- 可选扩展：`qitos[models]`、`qitos[hf]`、`qitos[all]`
